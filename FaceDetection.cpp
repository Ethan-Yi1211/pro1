...
cv::Mat QImageToMatFace(const QImage& image)
{
    cv::Mat mat;
    switch (image.format())
    {
...
QImage MatToQImageFace(const cv::Mat& mat)
{
    QImage image;
    if (mat.channels() == 1)
    {
...
public:
    ...
    FaceDetection(cv::Mat mainImage){
        this->mainImage = mainImage;
    }
    FaceDetection(QImage mainQ){
        this->mainQImage = mainQ;
        this->mainImage = QImageToMatFace(mainQImage);
    }
    ...
    QImage GetImage(){
        ...
        faceCascade.detectMultiScale(mainImage,faces,1.1,10);
        ...
        for(int i = 0; i<faces.size(); i++){
            ...
            cv::putText(mainImage, "Face", textPosition, cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255 - i * 25, i * 25, i * 25), 2);
            qDebug() << i;
        }

        return MatToQImageFace(mainImage);
    }
};
