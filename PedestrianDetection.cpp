...
cv::Mat QImageToMatPedestrian(const QImage& image)
{
    cv::Mat mat;
    switch (image.format())
    {
...
QImage MatToQImagePedestrian(const cv::Mat& mat)
{
    QImage image;
    if (mat.channels() == 1)
    {
...
public:
    ...
    PedestrianDetection(cv::Mat mainImage){
        this->mainImage = mainImage;
    }
    PedestrianDetection(QImage mainQ){
        this->mainQImage = mainQ;
        this->mainImage = QImageToMatPedestrian(mainQImage);
    }

    QImage GetImage(){
        cv::cvtColor(mainImage, mainImage, cv::COLOR_BGRA2BGR);
        cv::CascadeClassifier faceCascade;
        faceCascade.load("C:/opencv4.1.1/opencv-4.x/opencv-4.x/data/haarcascades/pedestrian.xml");

        if(faceCascade.empty()){
           qDebug() << "XMl file not loaded";
        }
        std::vector<cv::Rect> faces;
        faceCascade.detectMultiScale(mainImage,faces,1.05, 2);
        ...
        for(int i = 0; i<faces.size(); i++){
            ...
            cv::putText(mainImage, "Pedestrian", textPosition, cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255 - i * 25, i * 25, i * 25), 2);
            qDebug() << i;
        }

        return MatToQImagePedestrian(mainImage);
    }
};
